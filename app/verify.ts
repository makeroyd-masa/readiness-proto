...
{
  const { profile } = run({ q_household: 'solo', q_vulnerability: 'none', q_geo: 'none', q_inventory: ['meds_record'], q_worry: 'cost' });
...
{
  const nearby = run({ q_household: 'caregiver', q_role: 'caregiver_nearby', q_vulnerability: 'none', q_geo: 'none', q_inventory: [], q_worry: 'distance' });
  const remote = run({ q_household: 'caregiver', q_role: 'caregiver_remote', q_vulnerability: 'none', q_geo: 'none', q_inventory: [], q_worry: 'distance' });
...
{
  const { visited } = run({ q_household: 'solo', q_vulnerability: 'none', q_geo: 'travel', q_inventory: [], q_worry: 'not_sure' });
  ok('seam_travel is visited when geoRisk=travel', visited.includes('seam_travel'));
  const noTravel = run({ q_household: 'solo', q_vulnerability: 'none', q_geo: 'none', q_inventory: [], q_worry: 'not_sure' });
...
{
  const { profile } = run({ q_household: 'kids', q_vulnerability: 'young_kids', q_geo: 'disaster', q_inventory: ['meds_record', 'contacts'], q_worry: 'chaos' });
...
{
  const p0 = run({ q_household: 'couple', q_vulnerability: 'none', q_geo: 'none', q_inventory: ['meds_record', 'contacts', 'decision_maker', 'go_bag', 'written_plan'], q_worry: 'not_sure' }).profile;
